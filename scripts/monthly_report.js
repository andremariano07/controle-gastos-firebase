...
function buildAlertText(month) {
  ...
  if (month.percentualSaidas > ALERT_PERCENT) {
    return `🚨 <b>Alerta:</b> suas saídas consumiram <b>${pct(month.percentualSaidas)}</b> do salário, acima do limite de <b>${ALERT_PERCENT}%</b>.`;
  }
  return `✅ Suas saídas ficaram dentro do limite: <b>${pct(month.percentualSaidas)}</b> do salário (limite: ${ALERT_PERCENT}%).`;
}
...
function buildFullReport({ closedMonth, previousMonth, yearStats, trend, year }) {
...
• Mês mais apertado (menor sobra): ${monthIdToFullLabel(yearStats.piorMes.id)} — ${money(yearStats.piorMes.sobra)}`
  ...
  const alerta = buildAlertText(closedMonth);
...
async function main() {
  ...
  // As três leituras são independentes entre si — rodam em paralelo em vez
  // de sequencialmente, o que reduz a latência total da execução.
  const [closedMonth, previousMonth, yearMonths] = await Promise.all([
  ...
  const message = buildFullReport({ closedMonth, previousMonth, yearStats, trend, year });
